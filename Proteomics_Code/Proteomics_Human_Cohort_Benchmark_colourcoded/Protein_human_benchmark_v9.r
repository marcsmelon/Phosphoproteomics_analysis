...
COL_FDR     <- "#44DDF1"   # FDR < 0.05 but below FC cutoff (cyan)
...
# Transparent + blank background theme (appended to all ggplots)
# No grid lines, no panel border, fully transparent
...
# ---- VAD-only DCM (exclude myocarditis + unknown, keep only -V columns) ----
dcm_vad_only <- setdiff(all_vad_cols, c(myocarditis_cols, unknown_cols))

# ==============================================================================
# DEFINE 9 COMPARISONS
# ==============================================================================
...
comparisons <- list(
  ...
  C9 = list(d = dcm_vad_only, h = healthy_cols,
            label = "C9: DCM VAD-only (20 vs 14)",
            dir   = "C9_DCM_VAD_only")
)
...
generate_qc_plots <- function(log2_mat, norm_mat, condition, sample_labels,
  ...
  # ---- Density plot ----
  tryCatch({
    png(file.path(out_path, "density_plot_normalized.png"), width = 800, height = 600, bg = "transparent")
...
generate_pca_mds <- function(norm_mat, condition, sample_labels,
  ...
  tryCatch({
    ...
    png(file.path(out_path, "mds_plot.png"), width = 1000, height = 800, bg = "transparent")
  ...
  tryCatch({
    if (!requireNamespace("uwot", quietly = TRUE)) {
      cat("    ℹ UMAP skipped — install uwot: install.packages('uwot')\n")
    } else {
      library(uwot)
      
      # Remove rows with any NA
      complete_rows <- complete.cases(norm_mat)
      umap_mat <- norm_mat[complete_rows, ]
      
      if (nrow(umap_mat) > 50 && ncol(umap_mat) >= 6) {
        # UMAP on transposed matrix (samples as rows)
        set.seed(42)
        n_neighbors <- min(15, ncol(umap_mat) - 1)
        umap_result <- uwot::umap(t(umap_mat), n_neighbors = n_neighbors,
                                  min_dist = 0.3, metric = "euclidean",
                                  n_components = 2)
        
        umap_df <- data.frame(
          Sample    = colnames(umap_mat),
          Label     = sample_labels[colnames(umap_mat)],
          Condition = as.character(condition),
          UMAP1     = umap_result[, 1],
          UMAP2     = umap_result[, 2],
          stringsAsFactors = FALSE
        ...
        p_umap <- ggplot(umap_df, aes(x = UMAP1, y = UMAP2, color = Condition, label = Label)) +
          geom_point(size = 3, alpha = 0.8) +
          geom_text_repel(size = 2.5, max.overlaps = 20) +
          scale_color_manual(values = CONDITION_COLORS) +
          labs(title = paste0("UMAP — ", label),
        ...
        ggsave(file.path(out_path, "umap_plot.png"), p_umap,
               width = 10, height = 8, dpi = 300, bg = "transparent")
        
        write.csv(umap_df, file.path(out_path, "umap_coordinates.csv"), row.names = FALSE)
        cat("    ✓ UMAP plot + coordinates saved\n")
      }
    }
  }, error = function(e) cat(sprintf("    ⚠ UMAP: %s\n", e$message)))
}
...
run_comparison <- function(data_mat, gene_symbols, disease_cols, healthy_cols,
  ...
  tryCatch({
    cat("  Top DE heatmap (blood-excluded)...\n")
    
    # Select top DE proteins: 25 up in DCM + 25 down in DCM
    # Exclude blood markers and unmapped UniProt IDs
    de_clean <- top_table %>%
      filter(adj.P.Val < 0.05,
             !Gene %in% BLOOD_MARKER_GENES,
             !grepl("^[A-Z][0-9][A-Z0-9]{3}[0-9]", Gene),
             !is.na(Gene), Gene != "") %>%
      arrange(adj.P.Val) %>%
      filter(!duplicated(Gene))
    
    top_up <- de_clean %>% filter(logFC > 0) %>% slice_head(n = 25)
    top_down <- de_clean %>% filter(logFC < 0) %>% slice_head(n = 25)
    de_for_heatmap <- bind_rows(top_up, top_down)
    
    cat(sprintf("    Heatmap selection: %d up in DCM + %d down in DCM = %d total\n",
                nrow(top_up), nrow(top_down), nrow(de_for_heatmap)))
    ...
    if (nrow(de_for_heatmap) >= 5) {
      ...
      p_heat <- ggplot(melted, aes(x = Sample, y = Gene, fill = Z_score)) +
        ...
        labs(title = paste0("Top DE Proteins (blood-excluded) — ", label),
             subtitle = "Top 25 up + top 25 down by FDR | Disease – Healthy | blood-excluded",
             x = NULL, y = NULL) +
      ...
      # Width: enough room for gene labels + cells + legend
      plot_w <- max(14, n_samples * 0.28 + 6)
      plot_h <- max(10, n_genes * 0.28 + 3)
      
      ggsave(file.path(out_path, "heatmap_top_DE_no_blood.png"), p_heat,
             width = plot_w, height = plot_h,
             dpi = 300, bg = "transparent")
      cat(sprintf("  ✓ Heatmap saved (%d proteins × %d samples, blood excluded)\n",
                  n_genes, n_samples))
    } else {
      cat("  ℹ Too few non-blood DE proteins for heatmap (<5)\n")
    }
  }, error = function(e) cat(sprintf("  ⚠ Heatmap (top 50) FAILED: %s\n", e$message)))
  
  # ---- FULL DE heatmap: ALL proteins FDR < 0.05 (blood-excluded) ----
  tryCatch({
    cat("  Full DE heatmap (all FDR<0.05, blood-excluded)...\n")
    ...
    n_up_full   <- sum(de_full$logFC > 0)
    n_down_full <- sum(de_full$logFC < 0)
    cat(sprintf("    All DE (FDR<0.05, blood-excluded): %d proteins (%d up / %d down)\n",
                nrow(de_full), n_up_full, n_down_full))
    
    if (nrow(de_full) >= 10) {
      heat_proteins_full <- de_full$Protein
      heat_mat_full <- norm_mat[heat_proteins_full, , drop = FALSE]
      rownames(heat_mat_full) <- de_full$Gene
      
      # Z-score per row
      heat_z_full <- t(scale(t(heat_mat_full)))
      heat_z_full[is.na(heat_z_full)] <- 0
      
      melted_full <- reshape2::melt(as.matrix(heat_z_full))
      colnames(melted_full) <- c("Gene", "Sample", "Z_score")
      melted_full$Condition <- as.character(condition[as.character(melted_full$Sample)])
      
      # Order: genes by logFC (down at top, up at bottom)
      gene_order_full <- de_full %>% arrange(logFC) %>% pull(Gene)
      sample_order_full <- names(sort(condition))
      melted_full$Gene   <- factor(melted_full$Gene, levels = gene_order_full)
      melted_full$Sample <- factor(melted_full$Sample, levels = sample_order_full)
      
      n_genes_full   <- nrow(de_full)
      n_samples_full <- ncol(norm_mat)
      
      # For large heatmaps: suppress y-axis labels if too many genes
      show_y_labels <- n_genes_full <= 150
      
      p_heat_full <- ggplot(melted_full, aes(x = Sample, y = Gene, fill = Z_score)) +
        geom_tile(color = NA) +  # no cell borders for dense heatmaps
        scale_fill_gradient2(low = COL_HEALTHY, mid = "white", high = COL_DCM,
                             midpoint = 0, name = "Z-score") +
        facet_grid(. ~ Condition, scales = "free_x", space = "free_x") +
        labs(title = paste0("All DE Proteins FDR<0.05 (blood-excluded) — ", label),
             subtitle = sprintf("%d proteins (%d up in DCM / %d down) | ordered by logFC",
                                n_genes_full, n_up_full, n_down_full),
             x = NULL, y = NULL) +
        theme_minimal(base_size = 8) +
        theme(axis.text.x  = element_text(angle = 90, hjust = 1, vjust = 0.5, size = 5),
              axis.text.y  = if (show_y_labels) element_text(size = 3) else element_blank(),
      ...
      # Scale: fixed width, height proportional to gene count
      plot_w_full <- max(14, n_samples_full * 0.2 + 6)
      plot_h_full <- max(10, n_genes_full * 0.06 + 4)  # compressed row height for full set
      
      # Cap at reasonable max height
      if (plot_h_full > 60) plot_h_full <- 60
      
      ggsave(file.path(out_path, "heatmap_ALL_DE_FDR005_no_blood.png"), p_heat_full,
             width = plot_w_full, height = plot_h_full,
             dpi = 300, bg = "transparent", limitsize = FALSE)
      cat(sprintf("  ✓ Full DE heatmap saved (%d proteins × %d samples, %.0f × %.0f inches)\n",
                  n_genes_full, n_samples_full, plot_w_full, plot_h_full))
    } else {
      cat("  ℹ Too few DE proteins for full heatmap (<10)\n")
    }
  }, error = function(e) cat(sprintf("  ⚠ Full DE heatmap FAILED: %s\n", e$message)))
...
tryCatch({
  ...
  if (length(shared_genes) > 100) {
    ...
    png(file.path(OUTPUT_BASE_DIR, "comparison_logFC_dendrogram.png"), bg = "transparent",
