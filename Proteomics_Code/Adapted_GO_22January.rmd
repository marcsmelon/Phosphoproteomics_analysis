...
```{r heatmap_enrichment, fig.align="center", fig.width=12, fig.height=10, dpi=300, dev="png"}
library(dplyr)
library(pheatmap)
...
# Force use of original pheatmap package (not ComplexHeatmap wrapper)
pheatmap::pheatmap(top_data_scaled, 
         annotation_col = HManno_col,
         annotation_row = row_anno,
         annotation_colors = list(Treatment = treatment_colors, Regulation = c("Upregulated" = "#14767F", "Downregulated" = "#D55E00")),
         color = colorRampPalette(c("#301934", "#882255", "#8B4F9F", "#F1C232", "#F0E68C"))(100),
         ...
         show_rownames = TRUE,
         main = "Top 25 Up/Downregulated Proteins (p < 0.05)",
         filename = "results_24h/heatmap/Top_DE_Proteins_Heatmap.png",
         width = 10,
         height = 12)
...
pheatmap::pheatmap(top_data_scaled, 
         ...
         show_rownames = TRUE,
         main = "Top 25 Up/Downregulated Proteins (p < 0.05)",
         filename = "results_24h/heatmap/Top_DE_Proteins_Heatmap.pdf",
         width = 10,
         height = 12)

# Check files
list.files("results_24h/heatmap/")
...
```{r ERRB_enrichment, fig.align="center", fig.width=12, fig.height=10, dpi=300, dev="png"}

dir.create("results_24h/differential", recursive = TRUE, showWarnings = FALSE)


library(dplyr)
library(ggplot2)

# ERBB Signaling Pathway genes (KEGG hsa04012)
erbb_genes <- c(
  # Receptors
...
if(nrow(erbb_results) > 0) {
  ...
  if(nrow(erbb_results_unique) > 0) {
    ...
    # Save
    ggsave("results_24h/differential/ERBB_pathway_dotplot.svg", plot = p_erbb, width = 10, height = max(6, nrow(erbb_results_unique) * 0.3), device = "svg")
    ggsave("results_24h/differential/ERBB_pathway_dotplot.jpeg", plot = p_erbb, width = 10, height = max(6, nrow(erbb_results_unique) * 0.3), device = "jpeg", dpi = 300)
...
if(nrow(erbb_results) > 0) {
  ...
  if(nrow(erbb_results_unique) > 0) {
    ...
    # Save
    ggsave("results_24h/differential/ERBB_pathway_dotplot.svg", plot = p_erbb, width = 6, height = max(6, nrow(erbb_results_unique) * 0.3), device = "svg")
    ggsave("results_24h/differential/ERBB_pathway_dotplot.jpeg", plot = p_erbb, width = 6, height = max(6, nrow(erbb_results_unique) * 0.3), device = "jpeg", dpi = 300)
