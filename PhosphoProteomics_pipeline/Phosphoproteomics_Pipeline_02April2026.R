...
raw <- read.csv(PHOSPHO_FILE, header = TRUE, sep = "\t",
...
# ---- 9b: KSEA Barplot — Canonical style (gold/purple, 8×height) ----
plot_ksea_barplot_canonical <- function(ksea_df, contrast_name, output_dir, top_n = 25) {
  
  plot_df <- ksea_df %>%
    arrange(desc(activity)) %>%
    slice(c(1:min(top_n %/% 2, nrow(.)),
            max(1, nrow(.) - top_n %/% 2 + 1):nrow(.))) %>%
    mutate(
  ...
  p <- ggplot(plot_df, aes(x = activity, y = kinase, fill = direction)) +
    geom_col(alpha = 0.85) +
    geom_text(aes(
      x     = activity + sign(activity) * max(abs(plot_df$activity)) * 0.03,
      label = sig_label),
      size = 3.5, hjust = 0.5, fontface = "bold") +
    geom_text(aes(
      x     = 0,
      label = paste0(" n=", n_substrates, " "),
      hjust = ifelse(activity > 0, 1, 0)),
      size = 2.8, color = "grey40") +
    scale_fill_manual(values = c("Active" = COL_DCM, "Inactive" = COL_HEALTHY),
                      name = "Inferred state") +
  ...
  h <- max(5, nrow(plot_df) * 0.32 + 2)
  ggsave(file.path(output_dir, "ksea_barplot.png"), p,
         width = 9, height = h, dpi = 300, bg = "transparent")
  ggsave(file.path(output_dir, "ksea_barplot.pdf"), p,
         width = 9, height = h, bg = "transparent")
  cat("  ✓ KSEA barplot (canonical)\n")
}

# ---- 9c: KSEA Barplot — Compact style ----
plot_ksea_barplot_compact <- function(ksea_df, contrast_name, output_dir, top_n = 25) {
  ...
  p <- ggplot(plot_df, aes(x = activity, y = kinase, fill = direction)) +
    geom_col(alpha = 0.85, width = 0.7) +
    geom_text(aes(
      x     = activity + sign(activity) * max(abs(plot_df$activity)) * 0.04,
      label = sig_label),
      size = 5 / .pt, hjust = 0.5, fontface = "bold") +
    scale_fill_manual(values = c("Active" = COL_DCM, "Inactive" = COL_HEALTHY)) +
    geom_vline(xintercept = 0, linewidth = 0.2, color = "grey30") +
    labs(x = "KSEA z-score", y = NULL) +
    THEME_PLOT +
    theme(legend.position = "none")
  
  ggsave(file.path(output_dir, "ksea_barplot_compact.png"), p,
         width = PLOT_W, height = PLOT_H, units = "in", dpi = 300)
  ggsave(file.path(output_dir, "ksea_barplot_compact.pdf"), p,
         width = PLOT_W, height = PLOT_H, units = "in")
  cat("  ✓ KSEA barplot (compact)\n")
}

# ---- 9d: KSEA Volcano — Canonical style (scatter, size = n_substrates) ----
plot_ksea_volcano_original <- function(ksea_df, contrast_name, output_dir) {
  ...
  ggsave(file.path(output_dir, "ksea_volcano.png"), p,
         width = 8, height = 8, dpi = 300, bg = "transparent")
  ggsave(file.path(output_dir, "ksea_volcano.pdf"), p,
         width = 8, height = 8, bg = "transparent")
  cat("  ✓ KSEA volcano (canonical)\n")
}

# ---- 9e: KSEA Volcano — Compact style (theme_void, top 3, n= counts) ----
plot_ksea_volcano_compact <- function(ksea_df, contrast_name, output_dir) {
  ...
  ggsave(file.path(output_dir, "ksea_volcano_compact.png"), p,
         width = PLOT_W, height = PLOT_H, units = "in", dpi = 300)
  cat("  ✓ KSEA volcano (compact)\n")
}

# ---- 9f: Method comparison heatmap (gold/purple, PNG+PDF) ----
plot_kinase_method_heatmap <- function(kinase_res, output_dir, top_n = 25) {
  
  combined <- kinase_res$combined
  ksea     <- kinase_res$ksea_results
  if (is.null(combined) || is.null(ksea)) return(invisible(NULL))
  ...
  heatmap_df <- combined %>%
    ...
    select(kinase, all_of(method_cols)) %>%
    pivot_longer(-kinase, names_to = "method", values_to = "activity") %>%
    mutate(method = toupper(gsub("activity_", "", method))) %>%
    group_by(method) %>%
    mutate(activity_scaled = scale(activity)[, 1]) %>%
    ungroup()
  ...
  kinase_order <- ksea %>%
    ...
    arrange(activity) %>%
    pull(kinase)
  heatmap_df$kinase <- factor(heatmap_df$kinase, levels = kinase_order)
  
  n_methods <- length(unique(heatmap_df$method))
  n_kinases <- length(top_kinases)
  plot_w <- n_methods * 0.55 + 3.5
  plot_h <- n_kinases * 0.55 + 2.5
  
  p <- ggplot(heatmap_df, aes(x = method, y = kinase, fill = activity_scaled)) +
    geom_tile(color = "white", linewidth = 0.4) +
    coord_fixed(ratio = 1) +
    scale_fill_gradient2(low = COL_HEALTHY, mid = "white", high = COL_DCM,
                         midpoint = 0, name = "Activity\n(z-score)",
                         na.value = "grey85") +
    labs(title = "Kinase Activity: Method Comparison",
         subtitle = sprintf("n=%d kinases | ordered by KSEA z-score", n_kinases),
         x = NULL, y = NULL) +
  ...
  ggsave(file.path(output_dir, "kinase_method_comparison_heatmap.png"), p,
         width = plot_w, height = plot_h, dpi = 300, bg = "transparent")
  ggsave(file.path(output_dir, "kinase_method_comparison_heatmap.pdf"), p,
         width = plot_w, height = plot_h, bg = "transparent")
  cat("  ✓ Method comparison heatmap\n")
}

# ---- 9g: run_kinase_for_contrast — master wrapper ----
# Calls v14 run_kinase_activity (create_plots=FALSE to avoid old red/blue),
# then generates all plots externally with gold/purple + PNG/PDF,
# then calls run_kinase_pathway_analysis for fgsea + PROGENy.
run_kinase_for_contrast <- function(results, output_dir) {
  
  cdir <- file.path(output_dir, results$contrast_name, "kinase_activity")
  
  tryCatch({
    # ---- Run v14 kinase inference (no internal plots — we make our own) ----
    ...
    # ---- Generate all plots with gold/purple scheme ----
    # v14 returns: $ksea_results, $ulm_results, $wmean_results, $combined, etc.
    if (!is.null(kinase_res) && !is.null(kinase_res$ksea_results)) {
      ksea_df <- kinase_res$ksea_results
      
      # Barplots (canonical + compact)
      plot_ksea_barplot_canonical(ksea_df, results$contrast_name, cdir)
      plot_ksea_barplot_compact(ksea_df, results$contrast_name, cdir)
      
      # Volcanos (canonical + compact)
      # v14 KSEA columns: kinase, n_substrates, z_score, p_value, fdr, activity
      plot_ksea_volcano_original(ksea_df, results$contrast_name, cdir)
      plot_ksea_volcano_compact(ksea_df, results$contrast_name, cdir)
      
      # Method comparison heatmap
      plot_kinase_method_heatmap(kinase_res, cdir)
    }
    
    # ---- Kinase pathway analysis (fgsea + PROGENy) ----
    tryCatch({
      run_kinase_pathway_analysis(
      ...
      cat("  ✓ Kinase pathway analysis (fgsea + PROGENy)\n")
    }, error = function(e) {
      cat(sprintf("  ⚠ Kinase pathway analysis failed: %s\n", e$message))
    })
...
cat("    • kinase_activity/\n")
cat("      - ksea_results.csv, ulm_results.csv, wmean_results.csv\n")
cat("      - kinase_activity_all_methods.csv\n")
cat("      - ksea_barplot.png/.pdf  (canonical, gold/purple)\n")
cat("      - ksea_barplot_compact.png/.pdf\n")
cat("      - ksea_volcano.png/.pdf  (canonical 8×8)\n")
cat("      - ksea_volcano_compact.png/.pdf  (169×174 pt)\n")
